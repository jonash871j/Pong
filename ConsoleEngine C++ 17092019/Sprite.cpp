...
void Sprite::ImportLayer(std::string filename, int spriteLayer)
{
...
void Sprite::ReplaceLayer(int thisSpriteLayer, Sprite sprite, int spriteLayer)
{
	SPRITEMATRIX_START{
...
void Sprite::ReplaceSectionLayer(int x1, int y1, int x2, int y2, int thisSpriteLayer, Sprite sprite, int spriteLayer)
{
	for (int y = y1; y < y2; y++) {
		for (int x = x1; x < x2; x++) {
			charcterLayerArray[thisSpriteLayer][y][x] = sprite.charcterLayerArray[spriteLayer][y][x];
...
void Sprite::ChangeSpriteLayer(int spriteLayer)
{
	// Checks if layer isn't out of bounds
	if (spriteLayer > spriteAmount) return;
